/*
Rundeck / Runbook Automation API

Rundeck / Runbook Automation REST API for job automation, execution management, and system administration.  The Rundeck API provides comprehensive access to: - Job management (create, update, delete, execute jobs) - Execution monitoring and control - Project and resource management - Node filtering and resource queries - System configuration and administration - SCM integration (Git and other version control) - Authentication token management - Metrics and health monitoring  All API endpoints require authentication via API token, password session, or JWT token (Enterprise). API version must be specified in the URL path (e.g., /api/46/...).  For detailed documentation, see: [Rundeck API Docs](https://docs.rundeck.com/docs/api/)

API version: 59
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the UserErrorResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UserErrorResponse{}

// UserErrorResponse Error response for user operations
type UserErrorResponse struct {
	// Error message describing what went wrong
	Err *string `json:"err,omitempty"`
}

// NewUserErrorResponse instantiates a new UserErrorResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserErrorResponse() *UserErrorResponse {
	this := UserErrorResponse{}
	return &this
}

// NewUserErrorResponseWithDefaults instantiates a new UserErrorResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserErrorResponseWithDefaults() *UserErrorResponse {
	this := UserErrorResponse{}
	return &this
}

// GetErr returns the Err field value if set, zero value otherwise.
func (o *UserErrorResponse) GetErr() string {
	if o == nil || IsNil(o.Err) {
		var ret string
		return ret
	}
	return *o.Err
}

// GetErrOk returns a tuple with the Err field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserErrorResponse) GetErrOk() (*string, bool) {
	if o == nil || IsNil(o.Err) {
		return nil, false
	}
	return o.Err, true
}

// HasErr returns a boolean if a field has been set.
func (o *UserErrorResponse) HasErr() bool {
	if o != nil && !IsNil(o.Err) {
		return true
	}

	return false
}

// SetErr gets a reference to the given string and assigns it to the Err field.
func (o *UserErrorResponse) SetErr(v string) {
	o.Err = &v
}

func (o UserErrorResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UserErrorResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Err) {
		toSerialize["err"] = o.Err
	}
	return toSerialize, nil
}

type NullableUserErrorResponse struct {
	value *UserErrorResponse
	isSet bool
}

func (v NullableUserErrorResponse) Get() *UserErrorResponse {
	return v.value
}

func (v *NullableUserErrorResponse) Set(val *UserErrorResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableUserErrorResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableUserErrorResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserErrorResponse(val *UserErrorResponse) *NullableUserErrorResponse {
	return &NullableUserErrorResponse{value: val, isSet: true}
}

func (v NullableUserErrorResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserErrorResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


